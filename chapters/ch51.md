...
# 51. Lựa chọn các thành phần cho pipeline: giản lược tác vụ
...
1. Phân loại một bức ảnh có bị phơi sáng quá mức hay không (như trong ví dụ trên)

> 2. Classifying whether an image was taken indoor or outdoor

2. Phân loại một bức ảnh được chụp trong nhà hay ngoài trời

> 3. Classifying whether an image contains a cat

Phân loại một bức ảnh có chứa một con mèo hay không

> 4. Classifying whether an image contains a cat with both black and white fur

Phân loại liệu một bức ảnh chứa một con mèo khoang đen trắng hay không

> 5. Classifying whether an image contains a Siamese cat (a particular breed of cat)

Phân loại liệu một bức ảnh có chứa một con mèo Siamese (tên một loại mèo)
...
Mỗi trong số các tác vụ này là một bài toán phân loại ảnh nhị phân: Bạn phải có một bức ảnh đầu vào, và cho ra hoặc 0 hoặc 1. Nhưng những tác vụ trong danh sách này có vẻ "dễ" hơn nhiều để một mạng neural có thể học. Bạn sẽ có thể huấn luyện những tác vụ này với ít mẫu huấn luyện hơn.
...
Học máy chưa có một định nghĩa chính thống nào về một tác vụ là dễ hay khó[16]. Với sự phát triển của học sâu và mạng neural đa tầng, chúng ta nói một tác vụ là "dễ" nếu có có thể được thực hiện với số lượng bước tính toán ít hơn (ứng với mạng neural nông), và "khó" nếu nó đòi hỏi nhiều bước tính toán hơn (đòi hỏi một mạng neural sâu hơn). Nhưng đây đều là các định nghĩa không chính thống.
...
Nếu bạn có thể lấy một tác vụ phức tạp, và chia nhỏ nó thành những tác vụ con đơn giản hơn, sau đó viết mã cụ thể cho từng tác vụ con, bạn đang cung cấp cho thuật toán một kiến thức tiền đề giúp nó học một tác vụ hiệu quả hơn.

![img](../imgs/C51_02.png)

> Suppose you are building a Siamese cat detector. This is the pure end-to-end architecture:

Giả sử bạn đang xây dựng bộ phát hiện một con mèo Siamese. Dưới đây là một kiến trúc thuần đầu-cuối:
...
Bước đầu tiên (bộ phát hiện mèo) xác định mọi con mèo trong bức ảnh.
...
Bước thứ hai đưa những bức ảnh được cắt ra từ bộ phát hiện mèo (từng bức một) vào bộ phân loại mèo, và cuối cùng đưa ra 1 nếu bất kỳ ảnh cắt ra được xác định là một con mèo Siamese.
...
Bằng cách sử dụng pipeline này, bạn đang nói với thuật toán rằng có 3 bước chính để lái xe: (1) Phát hiện những chiếc xe khác, (2) Phát hiện người đi bộ, và (3) Hoạch định đường đi cho xe của bạn. Ngoài ra, mỗi bước này là một hàm số tương đối đơn giản hơn -- và có thể được học với ít dữ liệu hơn -- so với hướng tiếp cận thuần đầu-cuối.
...
Tóm lại, khi lựa chọn các thành phần cho một pipeline, hãy cố gắng xây dựng một pipeline mà mỗi thành phần là một hàm số tương đối "đơn giản" sao cho nó có thể được học từ chỉ một lượng dữ liệu vừa phải.
...
[17] Nếu bạn quen với các thuật toán phát hiện vật thể trên thực tế, bạn sẽ nhận ra rằng chúng không chỉ học với ảnh có nhãn 0/1, và thay vào đó được huấn luyện với khung chứa được cung cấp trong dữ liệu huấn luyện. Thảo luận về vấn đề này 
nằm ngoài phạm vi của cuốn sách. Tham khảo khóa "Deep Learning specialization" trên Coursera (http://deeplearning.ai) nếu bạn muốn học thêm về thuật toán này.
